...
// Removed duplicate declaration of graphicsLayer
        ...
        view.on("click", async (event) => {
          ...
          if (result) {
            ...
            const origin = result.graphic.attributes?.feature_origin;
            if (origin === 1) {
              setDrawnGeometry(result.graphic.geometry); // treat as drawn to trigger simplified form
            } else {
              setDrawnGeometry(null); // treat as existing
            }
            setOpen(true);
          }
        });
